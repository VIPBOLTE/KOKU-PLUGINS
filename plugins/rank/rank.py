...
# Watcher for overall and weekly messages
...
def _watcher(_, message):
    try:
        ...
        # Update total and weekly messages in the database
        rankdb.update_one({"_id": user_id}, {"$inc": {"total_messages": 1, "weekly_messages": 1}}, upsert=True)

        # Reset weekly data every Sunday
        current_day = datetime.now().weekday()  # 0: Monday, 6: Sunday
        if current_day == 6:  # If it's Sunday
            # Reset weekly data for all users
            rankdb.update_many({}, {"$set": {"weekly_messages": 0}})
...
async def weekly(_, message):
    try:
        top_members = rankdb.find().sort("weekly_messages", -1).limit(10)

        response = "⬤ 📈 ᴡᴇᴇᴋʟʏ ʟᴇᴀᴅᴇʀʙᴏᴀʀᴅ\n\n"
        users_data = []
        for idx, member in enumerate(top_members, start=1):
            user_id = member["_id"]
            total_messages = member["weekly_messages"]
            try:
                user_name = (await app.get_users(user_id)).first_name
            except:
                user_name = "Unknown"
